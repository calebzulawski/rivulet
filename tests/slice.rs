use rivulet::{slice::Slice, View, ViewMut};

#[test]
fn slice() {
    let storage: Vec<u8> = (0..100).collect();
    let mut stream = Slice::new(storage);
    ...
    // Check that the inner slice contains the original with the modification
    let storage = stream.into_inner();
    let expected = {
